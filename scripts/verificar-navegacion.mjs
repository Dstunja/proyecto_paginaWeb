 ...
 *      el build lleva PUBLIC_GA_ID, y los destinos permitidos de connect-src
 ...
 *   PUBLIC_GA_ID=G-TEST000000 npm run build
 *
...
function revisarCsp(csp, etiqueta) {
  ...
  comprobar(
    `${etiqueta}: CSP script-src sin 'unsafe-inline' ni 'unsafe-eval', con Turnstile y gtag`,
...
async function revisarCspEnNavegador(navegador, { base, local, etiqueta }) {
  ...
  // ---- Analítica ---------------------------------------------------------------
  {
    const respuestasGa = [];
    const alResponder = (r) => {
      if (/googletagmanager\.com\/gtag\/js|google-analytics\.com\/g\/collect/.test(r.url())) {
        respuestasGa.push(`${r.status()} ${new URL(r.url()).host}${new URL(r.url()).pathname}`);
      }
    };
    pagina.on('response', alResponder);
    await pagina.goto(`${base}/`, { waitUntil: 'load' });
    const conGa = (await pagina.content()).includes('googletagmanager.com/gtag/js');
    if (!conGa) {
      console.log(`  --    ${etiqueta}: analítica omitida — el build no lleva PUBLIC_GA_ID`);
    } else {
      await pagina.waitForTimeout(4000);
      const cargoGtag = await pagina.evaluate(() => typeof window.gtag === 'function' && Array.isArray(window.dataLayer));
      comprobar(
        `${etiqueta}: la analítica carga gtag.js sin bloqueos`,
        cargoGtag && respuestasGa.some((r) => r.includes('gtag/js')) && (await violaciones(pagina)).length === 0,
        respuestasGa.join(', ') || '(sin respuestas de Google)',
      );
    }
